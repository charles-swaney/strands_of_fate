...
class StatRequirement:
    ...
    def check(self, adventurer: "Adventurer") -> bool:
        return all(adventurer.base_stats.get(stat, 0) >= self.stat_reqs.get(stat, 0)
                   for stat in self.stat_reqs)
...
class JobLevelRequirement:
    ...
    def check(self, adventurer: "Adventurer") -> bool:
        return all(adventurer.levels_gained.get(job, 0) >= self.job_level_reqs.get(job, 0)
                   for job in self.job_level_reqs)
